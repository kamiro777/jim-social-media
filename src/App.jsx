...
function Sidebar({ month, setMonth }) {
  ...
  const labels = { '2026-04':'April 26','2026-05':'Mai 26','2026-06':'Juni 26','2026-07':'Juli 26','2026-08':'Aug 26','2026-09':'Sep 26' }
  return (
    <aside className="sidebar">
      ...
      <div className="month-selector">
        ...
        <select value={month} onChange={e => setMonth(e.target.value)}>
          {months.map(m => <option key={m} value={m}>{labels[m]}</option>)}
        </select>
      </div>
...
function AppShell() {
  ...
  const titles = { '/': 'Dashboard', '/instagram': 'Instagram', '/youtube': 'YouTube', '/podcast': 'Podcast', '/todos': 'Team To-Dos', '/meeting': 'Meeting-Vorlage' }
  return (
    <div className="app-shell">
      <Sidebar month={month} setMonth={setMonth} />
      <main className="main-content">
        <header className="page-header">
          <h1>{titles[location.pathname] || 'JIM Social Media'}</h1>
          <div className="header-meta">
            <span className="month-badge">MAI 2026</span>
            <span className="team-badge">7 Personen</span>
          </div>
        </header>
